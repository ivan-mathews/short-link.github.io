...
//============================================================================//
	...
	public function setTimezone(){
		...
		$sql = $this->query("SET `time_zone` = '%s'", date('P'));
		
	//	$tz = User::I()->time_zone ? User::I()->time_zone : $this->options['TIME_ZONE'];
	...
	public function setLcMessages(){
		$this->mysqli->query("SET lc_messages = '{$this->options['lang_lc']}'");
		return $this;
	}
...
//		}
		...
		if(User::I()->admin){
			...
			Core::error($this->error(), $sql, true);
		}else{
			Core::error($this->error(), $sql);
		}
...
//============================================================================//
	 ...
	 * @return bool
	 */
	public function insert($table, $data, $skip_check_fields = false, $id=false){
		
		$time 	= number_format(microtime(true),4);
		$new_id = str_replace(array(',','.'),array('',''),$time);
		$id 	= empty($id) ? array('id'=>$new_id) : array('id'=>$id);
		...
		if($this->query($sql)) { return $new_id; }
	 ...
	 * @return bool
	 */
	public function insertOrUpdate($table, $data, $update_data = false, $id=false){
